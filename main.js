...
function clipsLoaded(data) {
    ...
    data.clips.forEach(function(clip) {
        var $iframe = $('<iframe src="" width="640" height="360" frameborder="0" scrolling="no" allowfullscreen="true">');
        $iframe.attr('src',clip.embed_url);
        $clips.append($iframe);
    });
}
   ...
   function embedgameimage(data) {
    ...
    $("#game-image").attr('src', data.games[0].box.large);
}